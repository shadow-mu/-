...
2. 例
   ...
   3.教材P102第15题
   I had a big trouble.If only I__to my parents'advice then(那个时候-过去). --B
   A.listened
   B.had listened
   C.should listen
   D.would listen
...
1. **请求、要求、命令、建议**等，若==request要求,insist坚持要求,suggest建议 (suggestion)==,**advise建议(advisable),= ,demand要求，order命令，propose请求(proposal),ask命令，desire要求，，，require要求**，等表示“请求、要求、命令、建议等”动词后要用“虚拟语气**(should)do 。**
...
2. 易错题辨析
   ...
   ```
   1.The teacher advises that Tom (make/makes)use of every minute at school.--make
   2.The teacher advises that Tom (don't make/doesn't make/not make)use of every minute at school.--don't make
