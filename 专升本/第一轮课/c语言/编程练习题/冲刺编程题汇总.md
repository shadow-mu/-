...
2. 求解得ax^2^+bx+c=0方程的根。由键盘输入a,b,c。假设a,b,c的值任意，并不保证b2-4ac≥0。需要在程序中进行判别，如果b^2^-4ac≥0，就计算并输出方程的两个实根，如果b^2^-4ac<0,就输出“此方程无实根”的信息。
...
13. **一个数如果恰好等于它的因子之和，这个数就被称为“完数”。例如，6的因子为1,2,3，而6=1+2+3，因此6是“完数”。编程实现判断键盘输入的一个数是否是完数，如果是完数，则输出“是”，否则输出“不是”。**
...
14. 求100~200间的全部素数（每输出10个素数换行）。
    ...
    //素数又叫质数有无限个。质数定义为在大于1的自然数中，除了1和它本身以外不再有其他因数
    #include <stdio.h>
    int main(){
        int i,j,f,m=1;
        for(i=100;i<=200;i++){
...
19. 有一个分数序列：2/1,3/2,5/3,8/5,13/8,21/13.….求这个序列的前20项之和
...
15. 求一个3*3的整型矩阵对角线元素之和。
    ...
    ```c
    //主对角线行列相同，副对角线相加为行数-1{3-1=2}
    #include <stdio.h>
    int main(){
        int a[3][3],sum=0,i,j，sum=0;
        for(i=0;i<3;i++){
            for(j=0;j<3;j++){
            	scanf("%d",&a[i][j]);
                if(i==j||i+j==3-1){//注意等于号别写成赋值
                    sum+=a[i][j];
                }
       		 }
        }
...
16. 编写程序，定义一个二维数组a[ 3 ] [ 4 ],输入每个元素，求这些元素的和
...
17. 定义一个二维整型数组[ 5 ] [ 5 ]，输入每个元素的值，求其中最大值及其下标。

    ```c
    #include <stdio.h>
    int main(){
        int a[5][5],i,j,h=0,l=0;
        for(i=0;i<5;i++){
            for(j=0;j<5;j++){
                scanf("%d",&a[i][j]);
                if(a[h][l]<a[i][j]){
        ...
        printf("h=%d,l=%d,max=%d",h+1,l+1,a[h][l]);
        return 0;
    }
    ```
...
3. 编写程序，输入一个字符串，分别输出其中数字字符，字母字符以及其它字符的个数
   ...
   int main(){
       ...
       while(a[i]!='\0'){
           if(a[i]>='0'&&a[i]<='9') s++;
           else if(a[i]>='A'&&a[i]<='Z'||a[i]>='a'||a[i]<='z') z++;
           else q++;
           i++;
       }
       printf("%d,%d,%d",s,z,q);
